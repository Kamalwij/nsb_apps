...
def amortize_fixed(principal: float, annual_rate: float, months: int, start_date: Optional[dt.date] = None) -> pd.DataFrame:
    ...
    for i in range(1, months + 1):
        ...
        rows.append({"Period": i,"Date": d + pd.DateOffset(months=i-1),"Payment": emi,"Interest": interest,"Principal": principal_comp,"Balance": bal,})
        if bal <= 1e-6:
            break
    return pd.DataFrame(rows)

def amortize_variable(principal: float, periods: List[Tuple[int, float]], start_date: Optional[dt.date] = None) -> pd.DataFrame:
    ...
    for seg_idx, (seg_months, annual_rate) in enumerate(periods, start=1):
        if months_remaining <= 0 or bal <= 1e-6:
            break
        rate_m = annual_rate / 12.0 / 100.0
        emi = -pmt(rate_m, months_remaining, bal)
        for _ in range(seg_months):
            ...
            principal_comp = emi - interest
            bal = max(0.0, bal - principal_comp)
            rows.append({"Period": month_index,"Date": d + pd.DateOffset(months=month_index-1),"Segment": seg_idx,"Rate (annual %)": annual_rate,"Payment": emi,"Interest": interest,"Principal": principal_comp,"Balance": bal,})
            month_index += 1
            months_remaining -= 1
    return pd.DataFrame(rows)

def amortize_hybrid_teaser_then_fixed(principal: float, teaser_months: int, teaser_rate_annual: float, remaining_months_rate_annual: float, total_months: int, start_date: Optional[dt.date] = None) -> pd.DataFrame:
    d = start_date or dt.date.today().replace(day=1)
    periods = [(teaser_months, teaser_rate_annual),(max(0, total_months - teaser_months), remaining_months_rate_annual),]
...
def combine_tranche_schedules(schedules: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    for i, df in enumerate(schedules, start=1):
        ...
        d = d.rename(columns={"Payment": f"Payment_{i}","Interest": f"Interest_{i}","Principal": f"Principal_{i}","Balance": f"Balance_{i}",})
        norm.append(d)
    out = norm[0]
    for d in norm[1:]:
...
def solve_effective_rate_for_emi(total_principal: float, months: int, total_emi: float) -> float:
    ...
    for _ in range(80):
        mid = 0.5 * (lo + hi)
        pm = pmt(mid / 12.0, months, total_principal)
        if pm < target:
            hi = mid
        else:
            lo = mid
...
with st.sidebar:
    ...
    mode = st.selectbox("Mode",["Fixed","Variable (rate schedule)","Hybrid (teaser then fixed)","Tiered by amount (slabs)","Multi‑tranche (principal slabs with own schedules)","Tiered slabs (per‑slab teaser)"])
...
if mode == "Fixed":
    ...
    default_periods = [{"months": min(36, months), "annual_rate": 6.75},{"months": max(0, months - min(36, months)), "annual_rate": 9.50},]
    periods_df = st.data_editor(pd.DataFrame(default_periods),num_rows="dynamic",use_container_width=True,key="periods_editor",column_config={"months": st.column_config.NumberColumn("Months", min_value=1, step=1),"annual_rate": st.column_config.NumberColumn("Annual Rate (%)", min_value=0.0, step=0.05, format="%.2f"),},)
    ...
    default_slabs = [{"cap_lkr": 5_000_000, "annual_rate": 6.75},{"cap_lkr": None, "annual_rate": 9.50},]
    slabs_df = st.data_editor(pd.DataFrame(default_slabs),num_rows="dynamic",use_container_width=True,key="slabs_editor",column_config={"cap_lkr": st.column_config.NumberColumn("Cumulative cap (LKR)", min_value=0, step=100000),"annual_rate": st.column_config.NumberColumn("Annual Rate (%)", min_value=0.0, step=0.05, format="%.2f"),},hide_index=True).copy()
    ...
    default_tranches = [{"tranche": "A", "amount": min(5_000_000, loan_amt)},{"tranche": "B", "amount": max(0.0, loan_amt - min(5_000_000, loan_amt))}]
    tr_df = st.data_editor(pd.DataFrame(default_tranches),num_rows="dynamic",use_container_width=True,key="tranches_editor",column_config={"tranche": st.column_config.TextColumn("Tranche ID"),"amount": st.column_config.NumberColumn("Amount (LKR)", min_value=0.0, step=100000),},hide_index=True).copy()
    default_sched = [{"tranche": "A", "months": min(36, months), "annual_rate": 6.75},{"tranche": "A", "months": max(0, months - min(36, months)), "annual_rate": 9.50},{"tranche": "B", "months": months, "annual_rate": 9.50},]
    sch_df = st.data_editor(pd.DataFrame(default_sched),num_rows="dynamic",use_container_width=True,key="tranche_sched_editor",column_config={"tranche": st.column_config.TextColumn("Tranche ID"),"months": st.column_config.NumberColumn("Months", min_value=1, step=1),"annual_rate": st.column_config.NumberColumn("Annual Rate (%)", min_value=0.0, step=0.05, format="%.2f"),},hide_index=True).copy()
    tranche_schedules = []
    for row in tr_df.itertuples(index=False):
        t_id = str(row.tranche); amt = float(row.amount)
        if amt <= 0: continue
        segs = sch_df[sch_df["tranche"] == t_id][["months", "annual_rate"]]
        if segs.empty:
            st.warning(f"No schedule rows for tranche '{t_id}'. Skipping."); continue
    ...
    default_slabs2 = [{"cap_lkr": 5_000_000, "teaser_months": 36, "teaser_rate": 6.75, "post_rate": 9.50},{"cap_lkr": None, "teaser_months": 0, "teaser_rate": 0.00, "post_rate": 9.50},]
    slabs2_df = st.data_editor(pd.DataFrame(default_slabs2),num_rows="dynamic",use_container_width=True,key="slabs2_editor",column_config={"cap_lkr": st.column_config.NumberColumn("Cumulative cap (LKR)", min_value=0, step=100000),"teaser_months": st.column_config.NumberColumn("Teaser months", min_value=0, step=1),"teaser_rate": st.column_config.NumberColumn("Teaser annual %", min_value=0.0, step=0.05, format="%.2f"),"post_rate": st.column_config.NumberColumn("Post‑teaser annual %", min_value=0.0, step=0.05, format="%.2f"),},hide_index=True).copy()
    slabs2_df = slabs2_df.sort_values(by=["cap_lkr"], na_position="last")
    tranches2 = []
    remaining = loan_amt; prev_cap = 0.0
    ...
    if tranche_schedules:
        sched = combine_tranche_schedules(tranche_schedules)
        eff_rate = solve_effective_rate_from_schedule(loan_amt, sched["Payment"].tolist()) if not sched.empty else 0.0
        meta = {"mode": "Tiered per‑slab teaser", "effective_rate": eff_rate,"emi": float(sched["Payment"].iloc[0]) if not sched.empty else 0.0}
    else:
        sched = pd.DataFrame()
...
with k1: st.metric("Loan Amount (LKR)", f"{loan_amt:,.0f}")
with k2: st.metric("Tenure (months)", f"{months}")
with k3: st.metric("Equivalent Single Rate (annual %)", f"{meta.get('effective_rate', 0.0):.2f}")
...
with c2:
    ...
    def summarize_schedule(df: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame([{
            "Total Payment": float(df["Payment"].sum()),
            "Total Interest": float(df["Interest"].sum()),
            "Tenure (months)": int(df["Period"].max()),
    ...
    if not sched.empty:
        ...
        if st.button("Export to Excel (.xlsx)"):
            with pd.ExcelWriter(fn, engine="openpyxl") as writer:
                ...
                }]).round(2)).to_excel(writer, index=False, sheet_name="Summary")
            st.success(f"Saved: {fn}")
            st.caption("File saved in the app working directory.")
